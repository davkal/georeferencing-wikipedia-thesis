...
In \nameref{sec:visualization} I give a quick introduction into the charts being used to present the data, and that will be featured more prominently in the following chapter \nameref{ch:experiment}. 

The application is can be accessed over the internet\footurl{http://davkal.github.com/WP-contributions/}{2012}{02}{04}.
Its source code is hosted on GitHub\footurl{https://github.com/davkal/WP-contributions}{2012}{02}{04}.
...
In a second pass, the Wikipedia article of each country was retrieved to extract the country's  geographic coordinates\footnote{For some countries, coordinates were not present on the page, e.g. \weburl{http://en.wikipedia.org/wiki/Australia}{2012}{01}{24}. In that case, they were manually added by using that country capital's coordinates.}.
...
In a second step, the \term{List of official languages by state}\footurl{http://en.wikipedia.org/wiki/List_of_official_languages_by_state}{2012}{01}{29} was scraped and merged with the first list by matching the language name.\footnote{Not all countries have official languages---including the United States of America. In that case, the \term{de facto} official languages were used, e.g. English and Spanish for the United States of America.}
...
\begin{description}
    \item[query info] This basic query returns essential information like the article \abbr{ID}, the last revision \abbr{ID}, but also the full wikitext of the last revision.
...
The geographic lookup uses\footurl{https://github.com/volpino/toolserver-scripts/blob/master/\ac{PHP}/api.\ac{PHP}}{2012}{01}{24} the GeoCityLite database from Maxmind (see \nameref{sub:iplookup} for a discussion).
...
Treating Wikipedia's website as a system of pages, authors, and revisions, I modeled these as classes of objects using \entity{Backbone}.
...
Articles qualify that have revisions in at least 6 different months.
Revisions are grouped by month.
The hypothesis is supported if there is a linear dependence between the article age and the share of local contributions per month(see \nameref{sub:localness}), in the form of a negative correlation coefficient.
