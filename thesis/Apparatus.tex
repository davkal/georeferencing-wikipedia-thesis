...
The first section lists the \nameref{sec:datasources} and shows what type of content they provide for the article analysis.
In \nameref{sec:application}, I will give an overview of the application's architecture as well as the technologies being employed.

The section \nameref{sec:algorithms} describes the article requirements to qualify for an analysis. 
...
In \nameref{sec:visualization} I give an quick introduction into the charts being used to present the data, and that will be featured more prominently in the following chapter \nameref{ch:experiment}. 
...
In a second pass, the Wikipedia article of each country was retrieved to extract the country's for geographic coordinates\footnote{For some countries, coordinates were not present on the page, e.g. \weburl{http://en.wikipedia.org/wiki/Australia}{2012}{01}{24}. In that case, they were manually added by using that country capital's coordinates.}.
...
From the \term{List of ISO 639-1 codes}\footurl{http://en.wikipedia.org/wiki/List_of_ISO_639-1_codes}{2012}{01}{29}, all languages including their two-letter codes were extracted.
In a second step, the \term{List of official languages by state}\footurl{http://en.wikipedia.org/wiki/List_of_official_languages_by_state}{2012}{01}{29} was scraped and merged with first list by matching the language name.\footnote{Not all countries have official languages---including the United States of America. In that case, the \term{de facto} official languages were used, e.g. English and Spanish for the United States of America.}
...
Monthly database snapshots of all wikis run by the Wikimedia Foundation, including Wikipedia,  are publicly available\footurl{http://dumps.wikimedia.org}{2011}{12}{11} as database dump files in the \ac{XML} file format.
...
\subsection{MediaWiki \abbr{API}}\label{sub:mediawikiapi}

Wikipedia runs on the open source software MediaWiki, written in \ac{PHP}.
It offers a well documented \ac{API}\footurl{http://www.mediawiki.org/wiki/API:Main_page}{2012}{01}{24} which can be used by other programs to remotely use the wiki's features such as changing content and restoring revisions.\footnote{The full capability of the \ac{API} can be seen at tried at the \emph{Sandbox} at \weburl{https://en.wikipedia.org/wiki/Special:ApiSandbox}{2012}{01}{24}, a recent addition to the MediaWiki software.}
...
Similar to MediaWiki's Special:Export page\footnote{The page \weburl{https://en.wikipedia.org/wiki/Special:Export}{2011}{12}{11} allows for exporting of articles from the English Wikipedia.}, the \ac{API} also offers an article export that includes all revisions.
\lstset{caption={Example \abbr{JSON} response to a query to list all bots that edited the article \emph{2011-2012 Bahraini uprising}},label=apicall}
\begin{lstlisting}
...
\begin{description}
    \item[query info] This basic query is returns essential information like the article \abbr{ID}, the last revision \abbr{ID}, but also the full wikitext of the last revision.
...
It offers an \ac{API}\footnote{The \ac{API} is documented here: \weburl{https://github.com/volpino/toolserver-scripts/tree/master/\ac{PHP}}{2011}{12}{12}} to get simple article statistics like article \abbr{ID}, text length, as well as complex data structures like a list of unique editors including their gender, if they are registered users and chose to reveal their gender in their Wikipedia account.\footnote{Try \weburl{http://toolserver.org/~sonet/api_gender.\ac{PHP}?article=Egypt&lang=en}{2011}{12}{11} to get a list of all registered users who edited the article \emph{Egypt} of the English Wikipedia.}
...
\lstset{caption={SoNet \abbr{API} response to a query for the article \emph{2011-2012 Bahraini uprising}},label=sonetapicall}
\begin{lstlisting}
	{
...
The geographic lookup is uses\footurl{https://github.com/volpino/toolserver-scripts/blob/master/\ac{PHP}/api.\ac{PHP}}{2012}{01}{24} the GeoCityLite database from Maxmind (see \nameref{sub:iplookup} for a discussion).
...
These statistics can be viewed through a browser\footnote{E.g. \weburl{http://stats.grok.se/en/201105/2011_Egyptian_Revolution}{2011}{12}{12}} or queried through an \ac{API} \footnote{E.g. \weburl{http://stats.grok.se/json/en/201105/2011_Egyptian_Revolution}{2011}{12}{12}}.\footnote{I built support for this data source into the application. It is disabled, however, as page views are out of scope regarding the content analysis.}
...
Given an article \abbr{ID} and a revision \abbr{ID}, the \ac{API} method \emph{wikimarkup} returns an annotated version of the wikitext of that revision.
An annotation consists of a trust value, the revision \abbr{ID} the text got introduced into the article as well as the authors user name or \ac{IP} address, e.g. revision 473029564 of the article \emph{2011-2012 Bahraini uprising}\footurl{http://en.collaborativetrust.com/WikiTrust/RemoteAPI?method=wikimarkup&pageid=30876395&revid=473029564}{2012}{01}{23}:
...
Following the impressive WikiTrip application design (see \nameref{sub:analysisprojects}) I decided to build a web application that runs entirely in a web browser.\footnote{Application development was done mainly using the Google Chrome browser. The application should run in any \ac{HTML}5-capable browser.}

\subsection{Technologies}

As a web application the software heavily relies on \ac{HTML} version 5, \ac{JS}, and \ac{CSS}. 
...
\begin{description}
  ...
  \item[jquery] The \ac{JS} library jQuery\footurl{http://jquery.com/}{2012}{01}{25} is used to asynchronously retrieve data from the various data sources, dynamically insert elements into the layout as well as parsing screen-scraped webpages by making use of its selectors for addressing elements.
  ...
  \item[Backbone] Built upon Underscore, Backbone\footurl{http://documentcloud.github.com/backbone/}{2012}{01}{25} provides a way to structure a \ac{JS} application.
  ...
  \item[lz77] A \ac{JS} implementation\footurl{https://github.com/olle/lz77-kit}{2012}{01}{25} of the LZ77 text compression algorithm\footurl{http://en.wikipedia.org/wiki/LZ77_and_LZ78}{2012}{01}{25}. 
...
Treating Wikipedia's website as a system of pages, authors, and revisions, I modeled these as classes of objects using \entity{Backbone} (for a complete overview of  all models, see \todo{model chart in appendix}).
...
For example, the \emph{article view} is rendered multiple times because it draws data from different sources (the page \abbr{ID} is available sooner than, say, the first sentence of the article). 
...
\begin{enumerate}
	...
	\item A call to \entity{parse} retrieves the \ac{HTML} version of the latest revision as well as the article's links to other language editions. 
...
\begin{enumerate}
    \item Fetch a list of articles by calling \entity{query categorymembers} or \entity{query embeddedin}.
    \item Run the article analyzer routine on all articles.
    \item Compute group results and show them in the browser.
\end{enumerate}

During the group analysis only the article overview will be rendered while all of the article analyzer's steps are being executed.
While \entity{query embeddedin} returns a one-dimensional list, \entity{query categorymembers} can return subcategories.
...
The annotations can be easily addressed with \ac{CSS} selectors\footnote{This requires the template to be properly used by the authors, e.g. \weburl{http://en.wikipedia.org/wiki/Eastern_Front_(World_War_II)}{2012}{01}{29} where  the template \term{Start date} is used for the start and the end date, producing the two dates with the same annotation: \verb"dtstart". Another source of error is the order of year, month, and date, e.g. ``Municipal Library Elevator Coup'' happened on 28 January 1908 which was added as  \verb"Start date|1908|28|01" to the info box but unpredictably rendered as April 1, 1908, see \weburl{http://en.wikipedia.org/wiki/Municipal_Library_Elevator_Coup}{2012}{01}{25}.} and the values conform to the standard for the representation of date and time, \ac{ISO} 8601\footurl{http://en.wikipedia.org/wiki/ISO_8601}{2012}{01}{25}.
...
When a date was extracted using my custom parser, its accuracy is also stored as the \term{date resolution} with one of the following values: \term{day}, \term{month}, \term{year}.
...
Some articles do not have coordinates, although they clearly describe an event, e.g. \emph{Maspero demonstrations}\footurl{http://en.wikipedia.org/wiki/Maspero_demonstrations}{2012}{01}{25}.
...
Most of the authorship processing is being done by SoNet's Toolserver script\footurl{https://github.com/volpino/toolserver-scripts/blob/master/\ac{PHP}/api.\ac{PHP}}{2012}{01}{25}, see \nameref{sub:toolserver}.
...
\subsubsection{Parsing user pages}\label{sub:parsinguserpages}
...
As accurate as the coordinates from the \ac{IP} location services may appear, adding the results of the user page analysis to the set of located authors means having to settle for a country-level resolution.
...
\citeauthor{hardy2011volunteered}'s formula for the signature distance uses basic properties of articles and authors.
...
With more data available, namely the attribution of text to individual authors (see \nameref{sub:authorshipalgorithm}), I can provide two variations of the signature distance metric.
...
When compared to the original signature distance, the following two metrics indicate whether more local or more distant contributions prevailed.
...
Following the previously used notation, I will develop the \term{e.surv signature distance} metric.
...
\end{align*}
This restriction on edit survival leads to the \term{e.surv signature distance} for revision $\pi$ of article $\alpha$:
...
This very fact allows for the original signature distance to be used as a baseline for the \term{e.surv index}:

\begin{align}
 I_{\text{e.surv}} &= \frac{D_{\text{e.surv}}(\alpha,\pi)}{D(\alpha)} \cdot 100 & D(\alpha) &\neq 0
\end{align}

This normalizes the \term{e.surv signature distance} to a baseline of 100.
...
The second variation, the \term{t.surv signature distance} metric, makes one further adjustment.
...
\end{align*}
This leads to the \term{t.surv signature distance} for revision $\pi$ of article $\alpha$:
...
Compared with the original signature distance $D$, a lower value for $D_{\text{t.surv}}$ indicates that text from local contributions fared better than the average contribution.
...
As in the previous section, I will introduce a new indicator for text survival, the \term{t.surv index}:
...
An essential part of the authorship analysis deals with the question whether an author is \term{local}.
...
\lstset{caption={Localness of an author},label=localness,language=Python,numbers=left}
\begin{lstlisting}
...
The next chapter, \nameref{ch:experiment}, discusses how the shown results can be interpreted.

%\begin{todos}
%    \item \fullcite{kjellin2010evaluating}
%    \item \abbr{ID}entify. Characteristics of an object.
...
Therefore, a second choropleth map (\imgref{survival}) is shown below the first one, displaying text volume  based on text survival (see also \nameref{sec:contribution}).
...
\imgwide{activity}{Localness chart for \emph{2011--2012 Bahraini uprising}}

As part of the article analysis, a timeline chart shows the evolution of the three proximity metrics over time: the signature distance and the two new variants, \term{e.surv} and \term{t.surv}, \imgref{activity}.
...
Even clearer than the choropleth map, the motion chart identifies the main contributor countries, e.g. the big blue bubble (Bahrain) and the red one (United States), \imgref{evolution}.
In the example, changing the metric on the y-axis to show the proportion of located text reveals that contributions from Pakistan have a higher survival rate (the yellow bubble moved up), \imgref{evolution2}. 
...
\subsection*{H1: Articles are created shortly after the start of the event.}
...
Some events have a precise date-to-date interval, e.g. \emph{2011 Dohuk riots}\footurl{http://en.wikipedia.org/wiki/2011_Dohuk_riots}{2012}{01}{27} while others merely happened over the course of a month, e.g. \emph{February 2010 Australian cyberattacks}\footurl{http://en.wikipedia.org/wiki/February_2010_Australian_cyberattacks}{2012}{01}{27}, or even years, e.g. \term{Oyster Wars}\footurl{http://en.wikipedia.org/wiki/Oyster_Wars}{2012}{01}{29}.
...
This allows to test whether a language edition corresponds to one of a country's official languages (see \entity{official languages} in \nameref{sub:wpwebsite}).

\subsection*{H4: Articles about political events are created by local authors.}

Articles qualify if their creator has been located.
An article lends support to the hypothesis if the creator is considered local, see \nameref{sub:localness}.
...
Articles qualify that have at least 10 late revisions.
The hypothesis is supported if there is a linear dependence between the article age and the share of anonymous contributions, in the form of a negative correlation coefficient.

\subsection*{H8: Later, the share of local contributions decreases over time.}

Articles qualify that have at least 10 late revisions that have been located.
The hypothesis is supported if there is a linear dependence between the article age and the share of local contributions (see \nameref{sub:localness}), in the form of a negative correlation coefficient.

\subsection*{H9: Local contributions are more likely to survive.}

Articles qualify that have least 10 located revisions.
Local contributions are more likely to survive when, on average, the \term{e.surv} signature distance is smaller than the original signature distance.
Using the \term{e.surv index}, the hypothesis is supported if for all revisions the mean \term{e.surv index} is smaller than 100.

\subsection*{H10: Text from local contributions is more likely to survive.}

Articles qualify that have least 10 located revisions.
Text from local contributions is more likely to survive when, on average, the \term{t.surv} signature distance is smaller than the original signature distance.
Using the \term{t.surv index}, the hypothesis is supported if for all revisions the mean \term{e.surv index} is smaller than 100.
...
It may be worth investigating whether revisions marked as minor edits\footnote{See \nameref{sub:revisionhistory}.} should be counted as full contributions.
...
One candidate for this are the WikiProject\footurl{http://en.wikipedia.org/wiki/Wikipedia:WikiProject}{2012}{01}{27} info boxes which registered users can put on their user pages.
From a box like \emph{Template:WikiProject New York City}\footurl{http://en.wikipedia.org/wiki/Template:WikiProject_New_York_City}{2012}{01}{27} an affiliation with New York City could be inferred.

The current algorithm is scanning for links to Wikipedia and then checks if they appear in the right context.
For prose that has not been formatted with links, e.g. ``I am from Warsaw.'' a web service like \entity{wikipediaminer}\footnote{Website: \weburl{http://wikipedia-miner.cms.waikato.ac.nz/}{2012}{01}{27}, Example ``I am from Warsaw.'': \weburl{http://wikipedia-miner.cms.waikato.ac.nz/demos/annotate/?source=I+am+from+Warsaw.}{2012}{01}{27}} could be used.
...
An implementation would load a user's contribution (MediaWiki \ac{API} query \entity{usercontribs}), fetch these pages and scan for coordinates, construct a convex hull around them and then determine the center.
