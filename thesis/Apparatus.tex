...
This chapter describes the data structures behind Wikipedia articles and the revision history as well as tools to retrieve and analyze those.

\section{Available data}

A lot of Wikipedia's content like articles, revisions, discussions is stored on its database servers.
Unfortunately, these databases are not directly accessible over the Internet.
The Wikimedia Foundation, however, makes a lot of the data available in the form of database dumps or through an API.
...
For each of the wikis a variety of dumps is available that include all articles and, optionally, their edit history, all categories, interwiki-links, etc.
These dump files can be quite large, e.g. a compressed dump of all articles of the English Wikipedia in their current revision has a size 7.3 GB.\footnote{The uncompressed size is 31.0 GB, see \weburl{http://en.wikipedia.org/wiki/Wikipedia:Database_download}{2011}{12}{11}}
...
When analyzing only a single article or a category articles, the MediaWiki API can deliver the same information contained in the dumps. 

\subsection{MediaWiki API}

Wikipedia runs on the server software MediaWiki which offers an API  to access wiki features, e.g. changing content and restoring revisions, as well as queries to article properties, e.g. revisions and categories.
The API also offers an article export including all revisions.\footnote{Similar functionality is also offered via MediaWiki's Special:Export page, e.g. \weburl{https://en.wikipedia.org/wiki/Special:Export}{2011}{12}{11} allows exporting articles from the English Wikipedia.}
...
\subsection{Toolserver}

The Germany based Wikimedia Deutschland e.V. operates Toolserver\footurl{http://toolserver.org}{2011}{12}{11}, a platform for software tools that can access a copy of Wikipedia's databases. 
The replicated databases include the English Wikipedia and major language editions.
However, access is restricted and requires an account on Wikimedia's Toolserver.
...
\item[Wiki Trip] JavaScript application\footurl{https://github.com/volpino/wikipedia-timeline}{2011}{12}{11} that uses the Wikipedia API as well as a toolserver backend to visualize the evolution of a single article over time including: anonymous vs. registered contributors, male vs. female registered users, anonymous edits by country.\footnote{Live demo: \weburl{http://sonetlab.fbk.eu/wikitrip/}{2011}{12}{11}} 
Its backend can be reused to identify the gender for registered users who revealed their gender in their account page.\footnote{Try \weburl{http://toolserver.org/~sonet/api_gender.php?article=Egypt&lang=en}{2011}{12}{11} to get a list of all registered users who edited the article \emph{Egypt} of the English Wikipedia.}
\item[] 
\end{description}
...
\item Page view statistics for a given article: \url{http://stats.grok.se/json/en/201105/2011_Egyptian_Revolution} using data from \url{http://dumps.wikimedia.org/other/pagecounts-raw/}
