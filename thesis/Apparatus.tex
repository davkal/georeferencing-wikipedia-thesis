...
Averaging the distances of authors weighted by their relative work leads to the signature distance $D(\alpha)$:
\begin{eqnarray}
D(\alpha) & = & \sum_{\forall \rho \in P(\alpha)} \frac{|\eta(\rho,\alpha)| \cdot \delta(\eta,\alpha)}{|N(\alpha)|} \\
 & = &  \sum_{\forall \rho \in P(\alpha)} \big\{w(\rho,\alpha) \cdot \delta(\eta,\alpha)\}
\end{eqnarray}

Given that an article has a location and authors that have been located, my algorithmic calculation of the signature distance is done as follows:
...
\begin{lstlisting}
	...
	for author in article.authors:
		if loc in author: # count only authors that have a location
			...
			edits = author.count # work
			total += edits # postpone relativization to last line 
			sd += dist * edits
	return sd / total
\end{lstlisting}

\todo{memoized version for all revisions}
...
A choropleth map is used to show how this measure varies over different countries.
...
Therefor, a second choropleth map is shown below the first one, displaying edit count based on text survival.
For this, the annotated markup provided by WikiTrust is parsed to extract the authors that introduced the text sequences that make up the final text.
