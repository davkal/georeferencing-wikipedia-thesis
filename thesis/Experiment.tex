...
In a final experiment, the two political articles \abbr{egypt-rev} and \abbr{orange-rev} were checked for hypothesis support.
...
pos=h,doinside=\small ]{Xll}{
    ...
    H6 Early: more local			& \HF 27\% local\tmark[c] 	&  \HF all distant  \\
    H7 Later: less anonymous		& \HF R = 0.0 	&  \HF R = 0.9  \\
    H8 Later: less local 			& \HF R = 0.0	& \HT 73\%\\
    H9 Local edit survival			& \HT $I_{\text{e.surv}} = 77.6$		& \HF R = 0.1\\
    H10 Local text survival 			& \HT $I_{\text{e.surv}} = 71.5$		& \HF  \\
    \bottomrule
}
